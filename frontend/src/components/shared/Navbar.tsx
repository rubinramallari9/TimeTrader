              ...
              <div id="user-menu" className="relative">
                ...
                {userMenuOpen && (
                  <div className="absolute right-0 top-11 w-48 bg-[#0E1520] border border-[#1E2D40] rounded-xl shadow-2xl py-1.5">
                    ...
                    {user.role === "repair" && (
                      <Link href="/dashboard/repairs" onClick={() => setUserMenuOpen(false)} className="block px-4 py-2.5 text-xs text-[#9E9585] hover:text-white hover:bg-white/5 transition-colors">My Repair Shop</Link>
                    )}
                    {(user.role === "seller" || user.role === "store" || user.role === "repair" || user.role === "admin") && (
                      <Link href="/dashboard/ads" onClick={() => setUserMenuOpen(false)} className="block px-4 py-2.5 text-xs text-[#9E9585] hover:text-white hover:bg-white/5 transition-colors">My Ads</Link>
                    )}
